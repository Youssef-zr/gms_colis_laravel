...
(function ($) {
  ...
  $(".nav-link .fa-bars").on("click", function () {
    if (!$("body").hasClass("sidebar-collapse")) {
      ...
      localStorage.setItem("sideStatus", "opened");
    }
  });

  // show perso repetition for planning
  $("#Périodicité").on("change", function () {
    if ($(this).val() == "Personnalisé") {
      $("#perso-repetition").slideDown(500);
    } else {
      $("#perso-repetition").slideUp(500);
    }
  });

  // show terminate at inputs
  $('input[name="se_termine"]').on("change", function () {
    if ($(this).val() == "après") {
      $("input[name='se_termine_le']").val("").attr('disabled',"disabled");
      $("input[name='se_termine_après']").removeAttr('disabled');
    } else if ($(this).val() == "le") {
      $("input[name='se_termine_après']").val("").attr('disabled',"disabled");
      $("input[name='se_termine_le']").removeAttr('disabled');
    } else {
      $("input[name='se_termine_après']").val("").attr('disabled',"disabled");
      $("input[name='se_termine_le']").val("").attr('disabled',"disabled");
    }
